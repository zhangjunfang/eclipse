...
;(function($){
	$.extend({
		"MessageBox_alert":function(title,msg,type){
			var img_msg = '<img  src= "../images/'+type+'.png" style="width:24px;height:26px;" > &nbsp;&nbsp;'+msg;
			if($("#dialogalert").length ==0){
				$("body").append('<div id="dialogalert"></div>');
				$("#dialogalert").html(img_msg);
				__createDialog(title,msg);
			}else{
				$("#dialogalert").html(img_msg);
				$("#dialogalert").dialog('option','title',title).dialog('open');
			}
		},
		"MessageBox_confirm":function(title,msg,callback){
			var img_msg = '<img  src= "../images/help.png" style="width:24px;height:26px;" > &nbsp;&nbsp;'+msg;
			if($("#dialogconfirm").length ==0){
				$("body").append('<div id="dialogconfirm"></div>');
				$("#dialogconfirm").html(img_msg);
				__createConfirm(title,msg,callback);
			}else{
				$("#dialogconfirm").html(img_msg);
				$("#dialogconfirm").dialog('option','title',title).dialog('open');
			}
		}
	});
	function __createDialog(title,msg){
		$( "#dialogalert" ).dialog({
			resizable: false,
			title : title,
			height:120,
	...
	function __createConfirm(title,msg,callback){
...
;(function() {
	$.fn.extend({
		"getFormValue" : function() {
			...
			// 布尔类型
			this.find('input[type=checkbox]:not([value])').each(
				function() {
					var name = $(this).prop('name');
			...
			this.find('input[type=checkbox][value]').each(function() {
				...
				if ($(this).prop('checked')) {
					//ret[name].push($(this).prop('value')[0]);
					ret[name] = $(this).prop('value')[0];
				}
			});
