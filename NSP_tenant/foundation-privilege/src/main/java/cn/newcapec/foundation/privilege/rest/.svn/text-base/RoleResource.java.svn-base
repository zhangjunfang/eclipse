...
public class RoleResource extends PrivilegeResource {
		...
		try {
			if (null != getJsonObject()) {
				JSONObject data = JSONTools.getJSONObject(getJsonObject(),
						"data");
				if (data != null) {
					String ids = JSONTools.getString(data, "roleids");
	 ...
	 * @param response
	 */
	public void addRoleUI(BaseRequest request, BaseResponse response) {
...
//		MultiDatasourceContextHelper.setDatasource(MultiDatasourceContext.bizDataSourceCardMysql);
		try {
			...
			if (null != getJsonObject()) {
				...
				if (null != role) {
					...
					msg.setSuccess(true);
					msg.setMsg("添加角色成功！");
					roleService.saveOrUpdate(role,UserTool.getLoginUser());
					response.print(msg.toJSONObjectPresention());
				} else {
					msg.setMsg("添加角色失败！");
					response.print(msg.toJSONObjectPresention());
	...
	public void openDistributionUI(BaseRequest request, BaseResponse response) {
		...
		try {
			...
			// 所有菜单
			context.put("menus", menuService.queryMenu(params, orderby));
			// 获取资源
			context.put("functions", resourceService
					.queryResource(params, null));
			// 获取已经授权的资源
			context.put("functionselected", resourceService
	...
	public void openDepartmentAuthorizeUI(BaseRequest request,
		...
		try {
			...
			// 获取顶层部门
			context.put("department", departmentService.queryDepartments(
					params, orderby));
			// 获取所有部门
